...
export class WeatherDetailPage {
    ...
    async fetchCity() {
        const { data, status } = await api.get(`${API_BASE}/${this.cityId}`);
    ...
    async updateCity(updatedData) {
        const { data, status } = await api.patch(`${API_BASE}/${this.cityId}`, updatedData);
    ...
    async deleteCity() {
        if (confirm(`Удалить город "${this.city?.city}"?`)) {
            const { status } = await api.delete(`${API_BASE}/${this.cityId}`);
            if (status === 204) {
    ...
    renderDetail() {
        ...
        document.getElementById('saveEditBtn').onclick = async () => {
            ...
            if (newData.wind && newData.humidity && newData.condition) {
                await this.updateCity(newData);
            } else {
        ...
        document.getElementById('deleteCityBtn').onclick = async () => {
            await this.deleteCity();
        };
    }
